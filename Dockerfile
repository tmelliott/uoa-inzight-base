...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-keys FCAE2A0E115C3D8A \
    ...
    && dpkg -i libssl.deb \
    && rm -f libssl.deb \
    && R -e "install.packages('shiny', repos='http://cran.rstudio.com/', lib='/usr/lib/R/site-library')" \
    && R -e "install.packages('rmarkdown', repos='http://cran.rstudio.com/', lib='/usr/lib/R/site-library')" \
    && R -e "install.packages('DT', repos='http://cran.rstudio.com/', lib='/usr/lib/R/site-library')" \
